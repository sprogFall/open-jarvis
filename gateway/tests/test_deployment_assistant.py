...
def test_shell_script_is_linux_friendly_and_contains_compose_actions():
    ...
    assert "docker compose up -d --build postgres gateway client dashboard" in script
    assert "检查并补全 .env" in script
    assert "python3 \"$SCRIPT_DIR/jarvisctl.py\"" not in script



def test_shell_script_can_fill_missing_env_file_from_prompts(tmp_path: Path):
    ...
    assert "OMNI_AGENT_DEVICE_KEY=device-pass-001" in rendered
    assert "OMNI_AGENT_DEVICE_KEYS=device-alpha=device-pass-001" in rendered
